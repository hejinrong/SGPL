...
parser.add_argument("-l2","--learning_rate2", type = float, default = 0.006) 
...
def train(im_width, im_height, depth):
    ...
    # 训练数据集
    f = h5py.File('data/UP/UP_' + str(im_width) + '_' + str(im_height) + '_' + str(depth) + '_support' + str(n_examples) + '.h5', 'r')
    ...
    for episode in range(EPISODE):
        ...
        # 打印新数组的形状
        # print("New shape:", new_one_hot_labels.shape)
    ...
    torch.save(feature_encoder.state_dict(),str('./model/UP_feature_encoder_9way_1shot_newmodel_' + str(LEARNING_RATE2)+'_' + str(n_examples) + 'FT_' + str(EPISODE) + '.pkl'))
    torch.save(Prototype_network.state_dict(),str('./model/UP_network_9way_1shot_newmodel_' +str(LEARNING_RATE2)+'_' + str(n_examples) + 'FT_' + str(EPISODE) + '.pkl'))
