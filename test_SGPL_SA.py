...
feature_encoder.load_state_dict(torch.load(str("./model/SA_feature_encoder_16way_1shot_newmodel_"+ str(LEARNING_RATE2)+"_5FT_1000.pkl")))
print("load feature encoder success")

Prototype_network.load_state_dict(torch.load(str("./model/SA_network_16way_1shot_newmodel_"+ str(LEARNING_RATE2)+"_5FT_1000.pkl")))
...
def test(im_width, im_height, channels):
    ...
    f = open(f"SA/SA_" + str(LEARNING_RATE2) + '_' + str(np.sum(np.trace(matrix)) / float(n) * 100) + ".txt", 'w')   
    ...
    # new_show_cropped = new_show[4:-4, 4:-4]
    ...
    plt.savefig(f"SA/SA_{LEARNING_RATE2}_"
