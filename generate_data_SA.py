...
img = loadmat('D:\HSI_Projects\hyperspectral_data\Salinas_corrected.mat')['salinas_corrected']
gt = loadmat('D:\HSI_Projects\hyperspectral_data\Salinas_gt.mat')['salinas_gt']
...
def preparation():
    ...
    f = open('./data/SA/gt_index_SA.txt', 'w')
    f1 = open('./data/SA/SA_label.txt', 'w')
    data = []
    label = []
    ...
    #
    f = h5py.File('.\data\SA\SA_' + str(PATCH_SIZE*2) + '_' + str(PATCH_SIZE*2) + '_100_test.h5', 'w')
    f['data'] = data
    f['label'] = label
    f.close()
    ...
    PATH = './data/SA/SA_' + str(PATCH_SIZE*2) + '_' + str(PATCH_SIZE*2) + '_100_support' + str(num_s) + '.h5'
    f = h5py.File(PATH, 'w')
