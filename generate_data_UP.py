...
img = loadmat('D:\HSI_Projects\hyperspectral_data\PaviaU\PaviaU.mat')['paviaU']
...
def preparation():
    ...
    f = open('./data/UP/gt_index_UP.txt', 'w')
    f1 = open('./data/UP/UP_label.txt', 'w')
    data = []
    label = []
    ...
    #
    f = h5py.File(r'.\data\UP\UP_' + str(PATCH_SIZE*2) + '_' + str(PATCH_SIZE*2) + '_100_test.h5', 'w')
    f['data'] = data
    f['label'] = label
    f.close()
    ...
    PATH = './data/UP/UP_' + str(PATCH_SIZE*2) + '_' + str(PATCH_SIZE*2) + '_100_support' + str(num_s) + '.h5'
    f = h5py.File(PATH, 'w')
